...
**Spec code (§7.20 Reference frame update process):**
```text
if frame_type == KEY_FRAME && show_frame == 1:
    refresh_frame_flags = (1 << NUM_REF_FRAMES) - 1
for i in 0..NUM_REF_FRAMES-1:
...
**Spec code (§7.8 Sign bias computation):**
```text
for i = 0..6:
    hint = RefOrderHint[ ref_frame_idx[i] ]
    RefFrameSignBias[LAST_FRAME+i] =
...
**Spec code (§7.8 Sign bias computation, §7.20 Update):**
```text
RefFrameSignBias[...] = ( get_relative_dist(hint, OrderHint) > 0 )
if (refresh_frame_flags >> 2) & 1:
    RefOrderHint[2] = OrderHint
```
...
**Spec code (§7.20 Reference frame update process):**
```text
if (refresh_frame_flags >> 7) & 1:
    RefOrderHint[7] = OrderHint
    RefValid[7] = 1
```
...
**Spec code (§7.8 Sign bias computation):**
```text
get_relative_dist(3,4) < 0 → bias 0
get_relative_dist(7,4) > 0 → bias 1
```
...
**Spec code (§7.20 Update):**
```text
if (refresh_frame_flags >> 4) & 1:
    RefOrderHint[4] = OrderHint
```
...
**Spec code (§7.21 Reference frame loading if `show_existing_frame` used later):**
```text
if show_existing_frame == 1:
    load from RefOrderHint[ frame_to_show_map_idx ]
```
