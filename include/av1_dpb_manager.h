...
public:
    ...
    DpbStatus begin_frame(const Av1FrameInputs& in,
                          ...
                          Av1BeginResult& out)
    {
        out = {}; out.pregrain_surf = out.postgrain_surf = out.display_surf = -1;
...
struct Av1BeginResult {
    Av1RefsResolved refs{}; // Surfaces for reference frames
    int work_surf{-1};      // Surface reserved for decoding this frame
    int display_surf{-1};   // Surface to display (-1 if none this AU)
};
