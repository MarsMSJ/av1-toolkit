...
I'll implement the AV1 decoder API layer with memory query and decoder creation functions.
...
Av1MemoryRequirements av1_query_memory(const Av1StreamInfo *info, 
    ...
    size_t queue_storage = (size_t)(queue_depth + 4) * sizeof(Av1FrameEntry);
...
Av1Decoder *av1_create_decoder(const Av1DecoderConfig *config) {
    ...
    // Initialize output queue (use same storage for simplicity, or allocate separate)
    Av1FrameEntry *output_storage = decoder->queue_storage + queue_capacity;
    int output_capacity = queue_capacity;
...
static void test_create_decoder(void) {
    ...
    // Allocate memory block (use aligned_alloc for proper alignment)
...
static void test_error_handling(void) {
    ...
    printf("\nTest 3: Memory too small\n");
    void *small_mem = malloc(1024);  // Way too small
    if (small_mem) {
