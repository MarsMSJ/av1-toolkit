...
int main(int argc, char *argv[]) {
    ...
    // Write dummy frames (minimal valid AV1 frame = sequence header + frame)
    // This is a minimal AV1 bitstream that decodes to a black frame
    // In production, you'd use actual encoded data
    
    // Minimal AV1 frame (just enough to not crash the decoder)
    // OBU: size(1) + type(1) + seq_header(1) = 3 bytes minimum
    uint8_t minimal_frame[] = {
        0x00,  // size = 0 (extension flag + obu_type)
        0x00,  // seq_header obu
        0x00   // end
    };
