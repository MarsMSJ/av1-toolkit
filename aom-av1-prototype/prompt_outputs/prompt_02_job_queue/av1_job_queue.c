#include "av1_job_queue.h"
#include <stdlib.h>
#include <string.h>
#include <time.h>
#include <errno.h>

#define NSEC_PER_USEC 1000ULL
#define USEC_PER_SEC  1000000ULL

int av1_frame_queue_init(Av1FrameQueue *q, Av1FrameEntry *storage, int capacity) {
    if (!q || !storage || capacity <= 0) {
        return -1;
    }

    memset(q, 0, sizeof(*q));
    q->entries = storage;
    q->capacity = capacity;
    q->head = 0;
    q->tail = 0;
    q->count = 0;

    if (pthread_mutex_init(&q->mutex, NULL) != 0) {
        return -1;
    }

    if (pthread_cond_init(&q->not_empty, NULL) != 0) {
        pthread_mutex_destroy(&q->mutex);
        return -1;
    }

    if (pthread_cond_init(&q->not_full, NULL) != 0) {
        pthread_cond_destroy(&q->not_empty);
        pthread_mutex_destroy(&q->mutex);
        return -1;
    }

    return 0;
}

int av1_frame_queue_push(Av1FrameQueue *q, const Av1FrameEntry *entry) {
    if (!q || !entry) {
        return -1;
    }

    pthread_mutex_lock(&q->mutex);

    if (q->count >= q->capacity) {
        // Queue is full - non-blocking, return error
        pthread_mutex_unlock(&q->mutex);
        return -1;
    }

    // Copy entry to the tail position
    memcpy(&q->entries[q->tail], entry, sizeof(*entry));

    // Advance tail (circular)
    q->tail = (q->tail + 1) % q->capacity;
    q->count++;

    // Signal consumer that queue is not empty
    pthread_cond_signal(&q->not_empty);

    pthread_mutex_unlock(&q->mutex);
    return 0;
}

int av1_frame_queue_pop(Av1FrameQueue *q, Av1FrameEntry *out, uint32_t timeout_us) {
    if (!q || !out) {
        return -1;
    }

    pthread_mutex_lock(&q->mutex);

    if (q->count == 0) {
        // Queue is empty
        if (timeout_us == 0) {
            // Non-blocking poll - return immediately
            pthread_mutex_unlock(&q->mutex);
            return -1;
        }

        // Calculate absolute timeout for pthread_cond_timedwait
        struct timespec ts;
        if (clock_gettime(CLOCK_REALTIME, &ts) != 0) {
            pthread_mutex_unlock(&q->mutex);
            return -1;
        }

        // Add timeout_us to current time
        uint64_t nsec = (uint64_t)ts.tv_nsec + (uint64_t)timeout_us * NSEC_PER_USEC;
        ts.tv_sec += (int)(nsec / (1000000000ULL));
        ts.tv_nsec = (long)(nsec % (1000000000ULL));

        // Wait for not_empty signal or timeout
        int ret = pthread_cond_timedwait(&q->not_empty, &q->mutex, &ts);

        if (ret == ETIMEDOUT) {
            pthread_mutex_unlock(&q->mutex);
            return -1;
        }

        if (ret != 0) {
            pthread_mutex_unlock(&q->mutex);
            return -1;
        }

        // After waking up, check again (spurious wakeup)
        if (q->count == 0) {
            pthread_mutex_unlock(&q->mutex);
            return -1;
        }
    }

    // Copy entry from head position
    memcpy(out, &q->entries[q->head], sizeof(*out));

    // Advance head (circular)
    q->head = (q->head + 1) % q->capacity;
    q->count--;

    // Signal producer that queue is not full
    pthread_cond_signal(&q->not_full);

    pthread_mutex_unlock(&q->mutex);
    return 0;
}

int av1_frame_queue_count(Av1FrameQueue *q) {
    if (!q) {
        return -1;
    }

    pthread_mutex_lock(&q->mutex);
    int cnt = q->count;
    pthread_mutex_unlock(&q->mutex);

    return cnt;
}

int av1_frame_queue_is_full(Av1FrameQueue *q) {
    if (!q) {
        return -1;
    }

    pthread_mutex_lock(&q->mutex);
    int full = (q->count >= q->capacity);
    pthread_mutex_unlock(&q->mutex);

    return full;
}

void av1_frame_queue_destroy(Av1FrameQueue *q) {
    if (!q) {
        return;
    }

    pthread_cond_destroy(&q->not_full);
    pthread_cond_destroy(&q->not_empty);
    pthread_mutex_destroy(&q->mutex);

    // Clear structure
    memset(q, 0, sizeof(*q));
}
