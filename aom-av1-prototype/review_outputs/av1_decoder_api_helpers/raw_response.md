...
## Progress
- [x] Focus area 1: calculate_frame_buffer_size — checked, 1 bug found
- [x] Focus area 2: find_free_dpb_slot / allocate_dpb_slot / release_dpb_slot — checked, 1 bug found
- [x] Focus area 3: find_pending_output / add_pending_output / remove_pending_output — checked, no bugs
- [x] Focus area 4: worker_thread_func / gpu_thread_func — checked, no bugs (stubs only)

---

## Focus Area 1: calculate_frame_buffer_size

### Bug 1.1: Incorrect chroma plane size calculation for 4:2:0 subsampling
- **File**: av1_decoder_api.c, line ~93
- **Problem**: The function uses a single `chroma_factor` (1.5 for 4:2:0) multiplied by the full luma dimensions. This incorrectly assumes chroma planes have the same dimensions as luma. For 4:2:0, chroma planes are width/2 × height/2, so the formula should account for this subsampling properly.
- **Fix**: Calculate luma and chroma sizes separately with proper subsampling applied to chroma dimensions.

---

## Focus Area 2: DPB slot functions

### Bug 2.1: DPB count never initialized, causing find_free_dpb_slot to always fail
- **File**: av1_decoder_api.c, line ~120
- **Problem**: The `dpb_count` field in `Av1Decoder` struct is never initialized. In `find_free_dpb_slot`, the loop iterates `for (int i = 0; i < decoder->dpb_count; i++)` — since `dpb_count` defaults to 0, the loop never executes and the function always returns -1. This means no DPB slots can ever be allocated.
- **Fix**: Initialize `dpb_count` to the actual array size (16) during decoder creation.

---

## Focus Area 3: Pending output functions

No bugs found. The implementation correctly:
- Returns -1 when all 16 slots are full (not undefined behavior)
- Properly checks the `valid` flag before accessing entries

---

## Focus Area 4: Thread functions

No bugs found in the context of this review. These are stubs that don't implement actual threading logic. The comments indicate "In a real implementation, this would process decode jobs" — so there's no shutdown flag to check yet.

---

### av1_decoder_api.c (corrected)
...
**Note**: Bug 2.1 (uninitialized dpb_count) requires a fix in the `av1_create_decoder` function which is not shown in the lines provided (1-376). The fix would be to add `decoder->dpb_count = 16;` during decoder initialization. The corrected code above includes the DPB struct definition for reference, but the actual initialization fix would be in the create function.