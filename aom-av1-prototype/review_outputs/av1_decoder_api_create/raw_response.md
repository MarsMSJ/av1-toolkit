...
## Progress
- [x] Focus area 1 — AOM API usage (aom_codec_init vs aom_codec_dec_init_ver)
- [x] Focus area 2 — Thread creation order
- [x] Focus area 3 — Error unwind path
- [x] Focus area 4 — Memory alignment check

---

## Focus Area 1: AOM API Usage

### Bug 1.1: Incorrect AOM decoder initialization function
- **File**: av1_decoder_api.c, line ~520
- **Problem**: The code uses `aom_codec_init()` which is incorrect for decoder initialization. The AOM API requires `aom_codec_dec_init_ver()` with the proper interface pointer and ABI version.
- **Fix**: Replace `aom_codec_init(&aom_codec_av1_dx, &aom_config, &decoder->aom_decoder)` with `aom_codec_dec_init_ver(&decoder->aom_decoder, &aom_codec_av1_dx, &aom_config, aom_config.init_flags, AOM_DECODER_ABI_VERSION)`

---

## Focus Area 2: Thread Creation Order
- **Checked**: Copy thread is created before worker threads (line ~549 vs ~573). This is correct.

---

## Focus Area 3: Error Unwind Path

### Bug 3.1: Unsafe thread cancellation in error path
- **File**: av1_decoder_api.c, lines ~585-592
- **Problem**: When worker thread N fails to create, the code uses `pthread_cancel()` on threads 0..N-1. This is unsafe because threads may be holding locks or in the middle of operations, leading to deadlocks or undefined behavior.
- **Fix**: Use a graceful shutdown mechanism instead of cancellation. Set a `running` flag to false and signal condition variables before joining.

---

## Focus Area 4: Memory Alignment Check

### Bug 4.1: Missing alignment validation for memory base
- **File**: av1_decoder_api.c, lines ~445-460
- **Problem**: `av1_query_memory()` returns alignment requirements in `req.alignment`, but `av1_create_decoder()` never validates that `config->memory_base` meets this alignment before passing it to `av1_mem_init()`.
- **Fix**: Add alignment check: `if (((uintptr_t)config->memory_base % req.alignment) != 0) { ... error ... }`
