...
## Code Review: Prompt 7 — av1_flush and av1_destroy_decoder

### Requirements Checklist

| # | Requirement | Status | Explanation |
|---|-------------|--------|-------------|
| 1 | Flush + drain cycle works (END_OF_STREAM returned correctly) | **PASS** | `av1_flush()` sets state to FLUSHING, `av1_sync()` returns AV1_END_OF_STREAM when in FLUSHING state and queue is empty |
| 2 | Destroy after flush is clean | **PASS** | `av1_destroy_decoder()` handles implicit flush, waits for copy jobs, joins all threads, releases resources |
| 3 | Destroy without flush doesn't hang | **PASS** | Implicit flush in destroy handles non-flushed state |
| 4 | Destroy with no decodes doesn't crash | **PASS** | Destroy handles empty queues and no pending output |
| 5 | No thread leaks (all threads joined) | **PASS** | Worker, GPU, and copy threads are properly joined/stopped |
| 6 | AddressSanitizer clean (no leaks) | **FAIL** | CRITICAL: Memory allocation/deallocation mismatch |

---

### Issues Found

#### CRITICAL Issue 1: Memory Allocation/Deallocation Mismatch

**File:** `av1_decoder_api.c`  
**Location:** `av1_destroy_decoder()` near end (around line 730)

**What's wrong:**
The decoder structure is allocated using `av1_mem_memalign()` in `av1_create_decoder()`:
```c
Av1Decoder *decoder = (Av1Decoder *)av1_mem_memalign(DEFAULT_ALIGNMENT, sizeof(Av1Decoder));
```

But in `av1_destroy_decoder()`, it's freed using standard `free()`:
```c
...
Since `av1_mem_set_override_enabled(false)` is called before `free(decoder)`, the memory allocator is switched back to standard malloc, but the memory was allocated from the custom pool. This causes:
- Memory corruption
- Potential crash
- AddressSanitizer will report this as an error

**How to fix it:**
Save the pointer to the decoder before disabling the override, or use a flag to track whether to use `av1_mem_free()` or `free()`.

---

#### CRITICAL Issue 2: AOM Decoder Iterator Not Reset in Flush

**File:** `av1_decoder_api.c`  
**Location:** `av1_flush()` function (around line 580)

**What's wrong:**
The AOM decoder iterator is not reset before calling `aom_codec_get_frame()` in the flush function. If `av1_flush()` is called multiple times, subsequent calls won't get remaining frames because the iterator is already at the end.
...
**How to fix it:**
Reset the iterator before the loop:
```c
...
#### WARNING Issue 3: Use-After-Free Risk in Destroy

**File:** `av1_decoder_api.c`  
**Location:** `av1_destroy_decoder()` (around line 725)

**What's wrong:**
The decoder structure is zeroed out with `memset()` before being freed:
```c
memset(decoder_ptr, 0, decoder_size);
free(decoder);
```

If any code tries to access `decoder` after this point (e.g., in debug prints or error paths), it will access freed memory. While the code appears to not use `decoder` after this, it's risky and unnecessary.

**How to fix it:**
Remove the security wipe, or do it after all operations are complete but before the final free.

---

#### STYLE Issue 4: Inconsistent Thread State Tracking

**File:** `av1_decoder_api.c`  
**Location:** Worker/GPU thread creation and cleanup

**What's wrong:**
The `running` flag is set in the thread function but checked in the main thread before join. However, the thread function sets `running = true` AFTER the printf, meaning there's a race condition where the main thread might see `running = false` even though the thread is running.
...
**How to fix it:**
Set `running = true` at the very beginning of the thread function, before any prints or other operations.
...
struct Av1Decoder {
    ...
    // Flag to track if decoder was allocated from custom allocator
    bool decoder_allocated_from_custom;
};
...
Av1Decoder *av1_create_decoder(const Av1DecoderConfig *config) {
    ...
    // Mark as allocated from custom allocator
    decoder->decoder_allocated_from_custom = true;
...
Av1DecodeResult av1_flush(Av1Decoder *decoder) {
    ...
    // FIX: Reset iterator before getting frames
...
int av1_destroy_decoder(Av1Decoder *decoder) {
    ...
    // Step 10: Save whether we need custom free before disabling override
    bool use_custom_free = decoder->decoder_allocated_from_custom;
    
    // Step 11: Clear the memory pool flag (restore normal malloc)
    av1_mem_set_override_enabled(false);
    av1_mem_shutdown();
    
    // Step 12: Free decoder structure using appropriate method
    // FIX: Use the correct free based on how decoder was allocated
    if (use_custom_free) {
        av1_mem_free(decoder);
    } else {
        free(decoder);
...
The other files (`av1_decoder_api.h` and `test_flush_destroy.c`) do not require corrections.