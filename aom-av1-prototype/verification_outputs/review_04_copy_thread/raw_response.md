...
## Code Review: AV1 Copy Thread

### Requirements Checklist

| Requirement | Status | Explanation |
|-------------|--------|-------------|
| Byte-identical copy (memcmp src vs dst) | **PASS** | `copy_job_execute` uses `memcpy` row-by-row with correct dimensions |
| Multiple sequential copies work | **PASS** | Circular queue with proper head/tail tracking; test verifies 4 sequential copies |
| Clean shutdown (no thread leak, no hang) | **PASS** | `destroy` sets `running=0`, broadcasts, and joins thread |
| ThreadSanitizer clean | **PASS** | Atomics used for status, mutex protects queue operations |
| Atomic status transitions are correct | **WARNING** | See issue below |

---

### Issues Found

#### 1. WARNING: `av1_copy_thread_wait` doesn't wait for PENDING jobs

**File:** `av1_copy_thread.c`  
**Location:** `av1_copy_thread_wait()` function, lines ~95-130

**What's wrong:**  
The wait function only waits when `status == AV1_COPY_IN_PROGRESS`. If a job is still `AV1_COPY_PENDING` (queued but not yet picked up by worker thread), the function returns -1 immediately instead of waiting.

```c
if (current_status == AV1_COPY_IN_PROGRESS) {
    // wait...
}
// If PENDING, falls through to return -1
```

This works "by accident" in the timeout test because it expects -1, but it's semantically incorrect.
...
#### 2. STYLE: Missing NULL check in worker thread

**File:** `av1_copy_thread.c`  
**Location:** `copy_thread_main()`, line ~180

**What's wrong:**  
After dequeuing, the code doesn't verify `job != NULL` before using it. While `enqueue` checks for NULL, defense in depth is good practice.

**How to fix it:**  
Add a NULL check after dequeuing.

---

#### 3. STYLE: Spurious wakeup handling in timed wait

**File:** `av1_copy_thread.c`  
**Location:** `av1_copy_thread_wait()`, line ~120

**What's wrong:**  
The timed wait loop checks `ret == ETIMEDOUT` but doesn't handle other return values (spurious wakeups) explicitly. The current `while` loop condition should handle this, but the logic could be clearer.

**How to fix it:**  
The current code is actually correct (the while loop re-checks the condition), but could be simplified.
...
int av1_copy_thread_wait(Av1CopyThread *ct, Av1CopyJob *job, uint32_t timeout_us) {
    ...
    if (timeout_us == 0) {
        ...
        while (atomic_load(&job->status) != AV1_COPY_COMPLETE) {
            ...
            /* Continue on spurious wakeups - loop will recheck condition */
        }
    }
...
static void test_four_copies(void) {
    ...
    /* Use separate buffers per job for proper verification */
...
/* Test 5: Timeout on wait - now tests actual timeout scenario */
static void test_wait_timeout(void) {
    printf("Test 5: Wait timeout\n");
    
    Av1CopyThread *ct = av1_copy_thread_create(1);
    ASSERT(ct != NULL, "Copy thread created");
    ...
    /* Enqueue and start first job */
    ASSERT(av1_copy_thread_enqueue(ct, &job1) == 0, "First job enqueued");
    
    /* Create a second job but don't enqueue it yet */
    Av1CopyJob job2 = {
        .frame_id = 1,
        .dpb_slot = 1,
        .src_planes = { src_y, src_u, src_v },
    ...
    /* Wait with short timeout on job2 (never enqueued) - should timeout */
    struct timespec start, end;
    clock_gettime(CLOCK_MONOTONIC, &start);
    
    int result = av1_copy_thread_wait(ct, &job2, 100000);  /* 100ms timeout */
    ...
    av1_copy_thread_wait(ct, &job1, 0);
    ASSERT(av1_copy_thread_get_status(&job1) == AV1_COPY_COMPLETE, "First job completed");
